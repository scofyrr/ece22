...
var defaultStreamHandler = defineHandlerCallback(
...
var GLOBAL_EVENT_STORAGE_KEY = /* @__PURE__ */ Symbol.for(
...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } =
    await import("./assets/_tanstack-start-manifest_v-Chhg3CAV.js");
...
var GLOBAL_STORAGE_KEY = /* @__PURE__ */ Symbol.for(
      ...
      let res = await (async () => {
        if (
          ...
          if (typeof serializedContext === "string")
            try {
              const deserializedContext = fromJSON(
    ...
    if (route.assets) {
      for (const asset of route.assets)
        if (asset.tag === "link" && asset.attrs?.href) {
          ...
          if (
            !(typeof rel === "string" ? rel.split(/\s+/) : []).includes(
              "stylesheet",
            )
          )
            continue;
...
var ServerFunctionSerializationAdapter = createSerializationAdapter({
  ...
  fromSerializable: ({ functionId }) => {
    const fn = async (opts, signal) => {
      return (await (await getServerFnById(functionId))(opts ?? {}, signal))
        .result;
    };
    return fn;
  },
...
async function loadEntries() {
  const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
    import("./assets/router-BQQ5DE_J.js").then((n) => n.r),
    import("./assets/start-CEtFPQFM.js"),
  ...
  const next = async (nextCtx) => {
    if (nextCtx) {
      ...
      for (const key of Object.keys(nextCtx))
        if (key !== "context") ctx[key] = nextCtx[key];
    }
    index++;
