...
function AuthProvider({ children }) {
  ...
  return /* @__PURE__ */ jsx(Ctx.Provider, { value: { user, session, profile, isAdmin, loading, refreshProfile }, children });
}
...
function NotFoundComponent() {
  return /* @__PURE__ */ jsx("div", { className: "flex min-h-screen items-center justify-center px-4", children: /* @__PURE__ */ jsxs("div", { className: "neo-card max-w-md p-8 text-center", children: [
    /* @__PURE__ */ jsx("h1", { className: "font-display text-7xl font-extrabold", children: "404" }),
    /* @__PURE__ */ jsx("h2", { className: "mt-4 font-display text-2xl font-bold", children: "Página no encontrada" }),
    /* @__PURE__ */ jsx("p", { className: "mt-2 text-sm text-muted-foreground", children: "Quizás se mudó o nunca existió." }),
    /* @__PURE__ */ jsx(Link, { to: "/", className: "neo-btn mt-6 inline-block rounded-lg bg-primary px-4 py-2 text-sm font-bold text-primary-foreground", children: "Volver al inicio" })
  ] }) });
}
...
const colegio = {
  ...
  fondoUrl: "https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&w=1920&q=80",
  hero: {
    titulo: "Colegio José Carlos Mariátegui",
    subtitulo: '"El Amauta"',
    descripcion: "Una comunidad educativa donde cada estudiante aprende enseñando. Conoce nuestra historia, valores y la red de trueque de conocimientos."
  },
  bio: {
  ...
  cta: {
    ...
    descripcion: "Publica lo que sabes, gana puntos y canjéalos por libros en la biblioteca digital.",
...
async function fetchPlatformContext(supabase2) {
  const [
    ...
    supabase2.from("profiles").select("full_name, grade, max_points, points, level").gt("max_points", 0).order("max_points", { ascending: false }).limit(10),
...
${posts.map((p, i) => {
        const who = p.profiles ? `${p.profiles.full_name} (${p.profiles.grade})` : "Autor desconocido";
        const cat = p.categories ? `${p.categories.emoji} ${p.categories.name}` : "Sin categoría";
...
${books.map((b, i) => {
        const who = b.profiles ? `${b.profiles.full_name} (${b.profiles.grade})` : "Autor desconocido";
...
${ranking.map((r, i) => `${i + 1}. ${r.full_name} (${r.grade}) — ${r.max_points} pts máx | ${r.points} pts actuales | Nivel ${r.level}`).join("\n")}`
  ...
  if (postsRes.error) sections.push(`(Aviso: error parcial al leer posts: ${postsRes.error.message})`);
  if (booksRes.error) sections.push(`(Aviso: error parcial al leer libros: ${booksRes.error.message})`);
...
const BLOCKED_USER_MESSAGE = "No puedo ayudar con ese tipo de contenido. Pregúntame sobre estudios, la plataforma o temas educativos.";
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const { data: userData, error: userErr } = await supabase2.auth.getUser();
        ...
        const { data: thread, error: thErr } = await supabase2.from("chat_threads").select("id, user_id").eq("id", payload.thread_id).maybeSingle();
        if (thErr || !thread || thread.user_id !== userId) {
          return jsonResponse({ error: "Hilo no encontrado" }, 404);
        }
        const { data: history } = await supabase2.from("chat_messages").select("role, content").eq("thread_id", payload.thread_id).order("created_at", { ascending: true }).limit(30);
        ...
        const [platformData, webPrefetch] = await Promise.all([
          ...
          messageNeedsWebHint(payload.message) ? searchWeb(payload.message.slice(0, 120)) : Promise.resolve(null)
        ]);
...
${webPrefetch}`;
        ...
        try {
          const result = await generateText({
            ...
            system: systemPrompt,
            messages: [...chatHistory, { role: "user", content: payload.message }],
            tools: {
              search_web: tool({
                description: "Busca en internet (Wikipedia + DuckDuckGo). Para actualidad o verificar hechos.",
          ...
          await supabase2.from("chat_threads").update({ updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", payload.thread_id);
