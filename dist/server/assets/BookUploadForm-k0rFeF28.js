...
function validateBookFile(file) {
  ...
  if (parts.length < 2) {
    return { ok: false, message: "El archivo debe tener extensión (ej. .pdf, .docx)" };
  }
  ...
  if (isBlockedExtension(ext)) {
    return { ok: false, message: `Formato .${ext} no permitido por seguridad (ejecutables/scripts)` };
  }
...
const BOOK_FILE_ACCEPT = ".pdf,.doc,.docx,.ppt,.pptx,.txt,.zip,.epub,.odt,.ods,.xls,.xlsx,.rtf,.csv,.mp3,.mp4,.png,.jpg,.jpeg,.webp";
...
function BookUploadForm({ asDialog, open = true, onClose, initialFile = null }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const { error: upErr } = await supabase.storage.from("book-pdfs").upload(filePath, file, { contentType: file.type || "application/octet-stream" });
      if (upErr) throw upErr;
      let coverUrl = null;
      if (cover) {
        const cext = cover.name.split(".").pop()?.toLowerCase() || "jpg";
        coverPath = `${user.id}/${crypto.randomUUID()}.${cext}`;
        const { error: cErr } = await supabase.storage.from("book-covers").upload(coverPath, cover, { contentType: cover.type });
        if (cErr) throw cErr;
  ...
  const input = "w-full rounded-lg border-2 border-ink bg-card px-4 py-3 outline-none focus:ring-2 focus:ring-primary";
  const canAfford = (profile?.points ?? 0) >= UPLOAD_FEE;
  const form = /* @__PURE__ */ jsxs("form", { onSubmit: submit, className: "space-y-5", children: [
    /* @__PURE__ */ jsx(
      "input",
      {
        ref: fileInputRef,
    ...
    /* @__PURE__ */ jsxs("div", { children: [
      /* @__PURE__ */ jsx("label", { className: "mb-1 block text-sm font-bold", children: "Archivo del libro" }),
      /* @__PURE__ */ jsxs(
        "button",
        {
          type: "button",
      ...
      file && /* @__PURE__ */ jsxs("p", { className: "mt-1 text-xs text-muted-foreground", children: [
    ...
    /* @__PURE__ */ jsxs("div", { children: [
      ...
      /* @__PURE__ */ jsx("input", { value: title, onChange: (e) => setTitle(e.target.value), className: input, maxLength: 200, required: true })
    ] }),
    /* @__PURE__ */ jsxs("div", { children: [
      /* @__PURE__ */ jsx("label", { className: "mb-1 block text-sm font-bold", children: "Autor (opcional)" }),
      /* @__PURE__ */ jsx("input", { value: author, onChange: (e) => setAuthor(e.target.value), className: input, maxLength: 120 })
    ] }),
    /* @__PURE__ */ jsxs("div", { children: [
      /* @__PURE__ */ jsx("label", { className: "mb-1 block text-sm font-bold", children: "Descripción (opcional)" }),
      /* @__PURE__ */ jsx("textarea", { value: description, onChange: (e) => setDescription(e.target.value), rows: 3, className: input, maxLength: 2e3 })
    ] }),
    /* @__PURE__ */ jsxs("div", { children: [
      /* @__PURE__ */ jsx("label", { className: "mb-1 block text-sm font-bold", children: "Portada (opcional)" }),
      /* @__PURE__ */ jsx("input", { type: "file", accept: "image/*", onChange: (e) => setCover(e.target.files?.[0] ?? null), className: input })
    ] }),
    /* @__PURE__ */ jsx(
      "button",
      {
        ...
        children: submitting ? "Subiendo..." : canAfford ? `Publicar (-${UPLOAD_FEE} pts)` : `Te faltan ${UPLOAD_FEE - (profile?.points ?? 0)} pts`
      }
    )
  ] });
  if (asDialog && open) {
    return /* @__PURE__ */ jsx("div", { className: "fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4", onClick: onClose, children: /* @__PURE__ */ jsxs(
      "div",
      {
        ...
        children: [
          /* @__PURE__ */ jsxs("div", { className: "mb-4 flex items-center justify-between", children: [
            /* @__PURE__ */ jsx("h2", { className: "font-display text-2xl font-extrabold", children: "Subir libro" }),
            /* @__PURE__ */ jsx("button", { type: "button", onClick: onClose, "aria-label": "Cerrar", children: /* @__PURE__ */ jsx(X, { className: "h-5 w-5" }) })
          ] }),
          profile && /* @__PURE__ */ jsxs("p", { className: "mb-4 inline-flex items-center gap-1 rounded-md border-2 border-ink bg-lemon px-2 py-1 text-sm font-bold", children: [
