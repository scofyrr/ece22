...
export const moderatePostServer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const [textVerdict, imgVerdict] = await Promise.all([
      ...
      data.imageUrl ? moderateImageAI(data.imageUrl) : Promise.resolve({ allowed: true } as const),
    ]);
    if (!textVerdict.allowed) return { allowed: false as const, message: textVerdict.userMessage };
    if (!imgVerdict.allowed) return { allowed: false as const, message: imgVerdict.userMessage };
...
export const moderateBookServer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const [textVerdict, imgVerdict] = await Promise.all([
      moderateTextAI(meta),
      data.coverUrl ? moderateImageAI(data.coverUrl) : Promise.resolve({ allowed: true } as const),
    ]);
    if (!textVerdict.allowed) return { allowed: false as const, message: textVerdict.userMessage };
    if (!imgVerdict.allowed) return { allowed: false as const, message: imgVerdict.userMessage };
    return { allowed: true as const };
