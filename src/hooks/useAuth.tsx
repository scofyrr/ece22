...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loadProfile = async (uid: string) => {
    ...
    const activeBan = (bans ?? [])[0] as { reason: string; until: string | null } | undefined;
    if (activeBan && (!activeBan.until || new Date(activeBan.until) > new Date())) {
  ...
  return (
    <Ctx.Provider value={{ user, session, profile, isAdmin, isTeacher, ban, loading, refreshProfile }}>
      {children}
    </Ctx.Provider>
  );
}
