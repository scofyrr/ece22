...
import { BOOK_FILE_ACCEPT, validateBookFile } from "@/lib/book-upload.constants";

export const Route = createFileRoute("/biblioteca")({
  head: () => ({ meta: [{ title: "Biblioteca Digital — JCM" }, { name: "description", content: "Canjea tus puntos por libros que la comunidad sube." }] }),
...
function BibliotecaCatalog() {
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase
      .from("books")
      .select("id,title,author,description,cover_url,price_points,owner_id,pdf_path")
    ...
    if (profile) {
      const { data: p } = await supabase.from("book_purchases").select("book_id").eq("buyer_id", profile.id);
  ...
  const buy = async (book: Book) => {
    ...
    setBusyId(book.id);
    const { error } = await supabase.rpc("purchase_book", { _book_id: book.id });
    setBusyId(null);
    if (error) {
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        <div className="mb-6 flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
          <div>
            <h1 className="font-display text-4xl font-extrabold">Biblioteca Digital</h1>
            <p className="text-muted-foreground">Sube tus libros y canjea puntos por los de otros estudiantes.</p>
            {profile && (
        ...
        {loading ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {[...Array(6)].map((_, i) => <div key={i} className="neo-card h-72 animate-pulse" />)}
          </div>
        ) : books.length === 0 ? (
          <div className="neo-card p-10 text-center">
            <BookOpen className="mx-auto h-10 w-10" />
            <p className="mt-3 font-display text-2xl font-bold">Aún no hay libros.</p>
            <p className="mt-1 text-muted-foreground">¡Sé el primero en compartir uno!</p>
            <button
              type="button"
              onClick={openUploadPicker}
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {books.map((b) => {
              ...
              return (
                <motion.div key={b.id} whileHover={{ y: -6 }} className="neo-card relative flex flex-col overflow-hidden">
                  {isTeacher && !isMine && (
                    <AlertDialog>
                      <AlertDialogTrigger asChild>
                        <button aria-label="Eliminar libro" className="neo-btn absolute right-2 top-2 z-10 rounded-full bg-destructive p-2 text-destructive-foreground shadow-md hover:scale-110">
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>"{b.title}" se eliminará permanentemente de la biblioteca.</AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction onClick={async () => {
                            ...
                            load();
                          }} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Sí, eliminar</AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                  )}
                  <div className="aspect-[3/4] w-full overflow-hidden border-b-2 border-ink bg-muted">
                    {b.cover_url ? (
                      ...
                      <div className="flex h-full items-center justify-center"><BookOpen className="h-12 w-12 text-muted-foreground" /></div>
                    )}
                  </div>
                  <div className="flex flex-1 flex-col p-4">
                    <h3 className="font-display text-lg font-extrabold leading-tight">{b.title}</h3>
                    {b.author && <p className="text-xs text-muted-foreground">por {b.author}</p>}
                    {b.description && <p className="mt-2 line-clamp-2 text-sm text-muted-foreground">{b.description}</p>}
                    <div className="mt-auto flex items-center justify-between pt-4">
                      <span className="chip bg-lemon"><Coins className="h-3 w-3" /> 20</span>
                      {isMine ? (
                        <span className="text-xs font-bold text-muted-foreground">Tu libro</span>
                      ) : isOwned ? (
                        <button onClick={() => download(b)} disabled={isBusy} className="neo-btn inline-flex items-center gap-1 rounded-lg bg-mint px-3 py-1.5 text-sm font-bold text-ink disabled:opacity-50">
                          <Download className="h-4 w-4" /> {isBusy ? "Abriendo..." : "Leer"}
                        </button>
                      ) : (
                        <button onClick={() => buy(b)} disabled={isBusy} className="neo-btn rounded-lg bg-primary px-3 py-1.5 text-sm font-bold text-primary-foreground disabled:opacity-50">
