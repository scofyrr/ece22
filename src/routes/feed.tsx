...
import DestacadosCarousel, {
  type DestacadoItem,
} from "@/components/DestacadosCarousel";
...
function FeedPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .limit(8)
      .then(({ data }) =>
        setDestacados((data ?? []) as unknown as DestacadoItem[]),
      );
  }, []);
  ...
  return (
    <div className="relative min-h-screen">
      ...
      >
        ...
        <div
          className="absolute right-0 top-1/2 h-80 w-80 rounded-full bg-andino-ocre/0 blur-3xl"
          style={{
            backgroundColor:
        ...
        />
        <div
          className="absolute left-1/3 bottom-0 h-96 w-96 rounded-full blur-[110px]"
          style={{
            backgroundColor:
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        <div className="mb-6 flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="text-muted-foreground">
              Lo último que la comunidad está enseñando.
            </p>
          </div>
          <Link
            to="/new"
            className="neo-btn rounded-lg bg-lemon px-4 py-2 font-bold text-ink glow-bloom"
          >
            + Publicar algo
          </Link>
        </div>
        ...
        {loading ? (
          <div className="grid gap-6 md:grid-cols-2">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="neo-card h-64 animate-pulse" />
            ))}
          </div>
        ) : posts.length === 0 ? (
          <div className="neo-card p-10 text-center">
            ...
            <Link
              to="/new"
              className="neo-btn mt-4 inline-block rounded-lg bg-primary px-4 py-2 font-bold text-primary-foreground"
            >
              Publicar
            </Link>
          </div>
