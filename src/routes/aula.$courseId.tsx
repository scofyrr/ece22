...
function CourseDetail() {
  ...
  const addWeek = async (title: string) => {
    ...
    const { error } = await (supabase as any)
      .from("course_weeks")
      .insert({ course_id: courseId, title, position: weeks.length });
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <div
        className="mb-6 rounded-2xl border-2 border-ink p-6 shadow-[4px_4px_0_0_var(--ink)]"
        style={{
          background: `linear-gradient(135deg, ${course.cover_color}22, ${course.cover_color}11)`,
        }}
      >
        <h1 className="font-display text-3xl font-black">{course.name}</h1>
        <p className="text-primary text-sm font-semibold uppercase mt-1">
          {course.school}
        </p>
        {iAmTeacher && (
          <p className="mt-2 text-xs">
            Código de acceso:{" "}
            <span className="chip bg-lemon">{course.join_code}</span>
          </p>
        )}
      </div>
      ...
      {iAmTeacher && (
        <button
          ...
          className="mb-4 neo-btn inline-flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-sm font-bold text-primary-foreground"
        >
      ...
      <div className="space-y-3">
        {weeks.length === 0 && (
          <div className="rounded-xl border-2 border-dashed border-ink/30 p-8 text-center text-muted-foreground">
            Aún no hay semanas.
          </div>
        )}
        {weeks.map((w) => {
          ...
          return (
            <div
              key={w.id}
              className="rounded-xl border-2 border-ink bg-card shadow-[3px_3px_0_0_var(--ink)]"
            >
              <button
                onClick={() => setOpenWeeks((s) => ({ ...s, [w.id]: !isOpen }))}
              ...
              >
                ...
                <span className="flex-1 font-bold text-lg uppercase">
                  {w.title}
                </span>
                {iAmTeacher && (
                  <span
                    ...
                    className="p-1 text-destructive hover:bg-muted rounded cursor-pointer"
                  >
                ...
                <div className="border-t-2 border-ink/10 px-4 py-3 space-y-2">
                  {wItems.map((it) => (
                    <ItemRow
                      ...
                      mySub={mySubs.find((s) => s.item_id === it.id)}
                      onDelete={() => delItem(it.id)}
                      onSubmit={() => setSubmitFor(it)}
                      onViewSubs={() => setSubsFor(it)}
                    />
                  ))}
                  {iAmTeacher && (
                    <button
                      onClick={() => setAddItemFor(w.id)}
                      className="w-full rounded-lg border-2 border-dashed border-ink/30 py-2 text-sm font-semibold text-muted-foreground hover:bg-muted"
                    >
      ...
      {addWeekOpen && (
        <AddWeekDialog onClose={() => setAddWeekOpen(false)} onSave={addWeek} />
      )}
      {addItemFor && (
        <AddItemDialog
          weekId={addItemFor}
      ...
      {subsFor && iAmTeacher && (
        <SubsPanel item={subsFor} onClose={() => setSubsFor(null)} />
      )}
    </div>
  );
}
...
function ItemRow({
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-ink/20 bg-background px-3 py-2">
      <div
        className={`flex h-9 w-9 items-center justify-center rounded ${color}`}
      >
      ...
      <div className="flex-1 min-w-0">
        ...
        {item.kind === "assignment" && item.due_at && (
          <div className="text-xs text-orange-600">
            Entrega: {new Date(item.due_at).toLocaleString()}
          </div>
        )}
      </div>
      <div className="flex items-center gap-2">
        ...
        {item.kind === "assignment" && iAmTeacher && (
          <button onClick={onViewSubs} className="chip bg-lemon">
            Ver entregas
          </button>
        )}
        {iAmTeacher && (
  ...
  return (
    <Modal onClose={onClose} title="Nueva semana">
      <input
        ...
        className="w-full rounded-lg border-2 border-ink px-3 py-2"
      />
      <div className="mt-4 flex justify-end gap-2">
        ...
        <button
          ...
          className="neo-btn rounded-lg bg-primary px-4 py-2 text-sm font-bold text-primary-foreground"
        >
          Guardar
        </button>
      </div>
...
function AddItemDialog({
  ...
  const submit = async () => {
    ...
    if (kind === "file" && file) {
      ...
      file_url = supabase.storage.from("course-materials").getPublicUrl(path)
        .data.publicUrl;
      file_name = file.name;
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  return (
    <Modal onClose={onClose} title="Nuevo recurso">
      <div className="mb-3 flex gap-2">
        {(["file", "video", "assignment"] as const).map((k) => (
          <button
            key={k}
            onClick={() => setKind(k)}
            className={`flex-1 rounded-lg border-2 border-ink px-3 py-2 text-xs font-bold ${kind === k ? "bg-primary text-primary-foreground" : "bg-card"}`}
          >
      ...
      <input
        ...
        placeholder="Título"
        className="mb-2 w-full rounded-lg border-2 border-ink px-3 py-2"
      />
      <textarea
        value={description}
      ...
      <div className="mt-3 flex justify-end gap-2">
        ...
        <button
          onClick={submit}
          disabled={saving}
          className="neo-btn rounded-lg bg-primary px-4 py-2 text-sm font-bold text-primary-foreground disabled:opacity-50"
        >
          Guardar
        </button>
      </div>
...
function SubmitDialog({ item, onClose }: { item: Item; onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    const file_url = supabase.storage.from("submissions").getPublicUrl(path)
      .data.publicUrl;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (supabase as any)
      .from("assignment_submissions")
...
function SubsPanel({ item, onClose }: { item: Item; onClose: () => void }) {
  const [rows, setRows] = useState<
    (Submission & { full_name?: string; dni?: string })[]
  >([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      setRows(
        subs.map((s) => ({
          ...s,
          full_name: map.get(s.student_id)?.full_name,
          dni: map.get(s.student_id)?.dni,
        })),
      );
    })();
  }, [item.id]);
  ...
  const downloadAll = async () => {
    ...
    a.href = url;
    a.download = `entregas_${item.title.replace(/\s+/g, "_")}.zip`;
    a.click();
    ...
    <Modal onClose={onClose} title={`Entregas — ${item.title}`}>
      <div className="mb-3 flex items-center justify-between">
        <span className="text-sm text-muted-foreground">
          {rows.length} entrega(s)
        </span>
        <button
          onClick={downloadAll}
          disabled={!rows.length || downloading}
          className="neo-btn inline-flex items-center gap-1 rounded-lg bg-primary px-3 py-1.5 text-xs font-bold text-primary-foreground disabled:opacity-50"
        >
        ...
        {rows.map((r) => (
          <GradeRow
            ...
            onSaved={(g, f) =>
              setRows((rs) =>
                rs.map((x) =>
                  x.id === r.id ? { ...x, grade: g, feedback: f } : x,
                ),
              )
            }
          />
...
function GradeRow({
  ...
  row: Submission & { full_name?: string; dni?: string };
  onSaved: (g: number | null, f: string | null) => void;
}) {
  ...
  const save = async () => {
    ...
    const { error } = await (supabase as any)
      .from("assignment_submissions")
      .update({ grade: g, feedback: feedback || null })
      .eq("id", row.id);
    setSaving(false);
    if (error) toast.error(error.message);
    else {
      toast.success("Calificación guardada");
      onSaved(g, feedback || null);
    }
  };
  return (
    <div className="rounded border border-ink/20 px-3 py-2 text-sm space-y-2">
      <div className="flex items-center justify-between gap-2">
        ...
        <a
          ...
          className="chip bg-lemon"
        >
          <Download className="h-3 w-3" /> {row.file_name}
        </a>
      </div>
      <div className="flex items-center gap-2">
        <input
          ...
          placeholder="Nota /20"
          className="w-20 rounded border-2 border-ink px-2 py-1 text-sm"
        />
        <input
          value={feedback}
          onChange={(e) => setFeedback(e.target.value)}
          placeholder="Comentario…"
          className="flex-1 rounded border-2 border-ink px-2 py-1 text-sm"
        />
        <button
          onClick={save}
          disabled={saving}
          className="neo-btn rounded bg-primary px-3 py-1 text-xs font-bold text-primary-foreground disabled:opacity-50"
        >
          Guardar
        </button>
      </div>
...
function Modal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg rounded-2xl border-2 border-ink bg-card p-6 shadow-[6px_6px_0_0_var(--ink)]"
