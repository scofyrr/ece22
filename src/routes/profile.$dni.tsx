...
import {
  Trophy, Sparkles, BookOpen, Ban, UserCheck, ShieldAlert, BarChart3,
} from "lucide-react";
import {
...
function ProfilePage() {
  ...
  const applyBan = async () => {
    ...
    const { error } = await (supabase as any).rpc("ban_user", {
      _user_id: profile.id, _reason: reason.trim(), _duration: duration,
    });
    setBanning(false);
    if (error) return toast.error(error.message);
    toast.success("Usuario baneado");
    setBanOpen(false); setReason("");
    load();
  };
  ...
  const unban = async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (supabase as any).rpc("unban_user", { _user_id: profile.id });
    if (error) return toast.error(error.message);
      ...
      <main className="mx-auto max-w-4xl px-4 py-8">
        {ban && (
          <div className="neo-card mb-4 flex items-center gap-3 border-destructive bg-destructive/10 p-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm">{ban.reason} {ban.until && <>· hasta {new Date(ban.until).toLocaleString("es")}</>}</p>
            </div>
          </div>
        )}
              ...
              <div className="flex flex-col gap-2">
                ...
                {ban ? (
                  ...
                  <AlertDialog open={banOpen} onOpenChange={setBanOpen}>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>Banear a {profile.full_name}</AlertDialogTitle>
                        <AlertDialogDescription>
                          El usuario no podrá publicar, comentar ni dar like durante el período seleccionado.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <div className="space-y-4">
                        ...
                        <div>
                          <label className="mb-1 block text-sm font-bold">Duración</label>
                          <div className="grid grid-cols-3 gap-2">
                            {([
                              ["1d", "1 día"], ["2d", "2 días"], ["1w", "1 semana"],
                              ["1m", "1 mes"], ["permanent", "Permanente"],
                            ] as const).map(([k, l]) => (
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction disabled={banning} onClick={applyBan} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
        ...
        {posts.length === 0 ? (
          ...
          <div className="mt-4 grid gap-6 md:grid-cols-2">
            {posts.map((p) => <PostCard key={p.id} post={p} onDeleted={(id) => setPosts((xs) => xs.filter((x) => x.id !== id))} />)}
          </div>
        )}
      </main>
    </div>
