...
function StudentDetail() {
  ...
  const load = async () => {
    ...
    const [
      ...
      supabase
        .from("posts")
        .select("id, title, created_at, category:categories(name,color)")
        .eq("author_id", uid)
      ...
      supabase
        .from("likes")
        .select("id, post_id, created_at, posts:posts(title)")
        .eq("user_id", uid)
        .order("created_at", { ascending: false }),
    ...
    const acts: Activity[] = [
      ...
      ...((posts ?? []) as any[]).map((x) => ({
        ...
        extra: x.category?.name,
      })),
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...((comments ?? []) as any[]).map((x) => ({
        kind: "comment" as const,
        id: x.id,
        text: x.content.slice(0, 200),
        created_at: x.created_at,
        extra: x.post_id,
      })),
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    setViews(
      groupByDay(
        (viewRows ?? []).map((r: { created_at: string }) => r.created_at),
      ),
    );
  ...
  if (!p)
    return (
      <div>
        <Navbar />
        <div className="mx-auto max-w-4xl p-8">Cargando...</div>
      </div>
    );
        ...
        <div className="neo-card mb-6 flex flex-col gap-4 p-6 md:flex-row md:items-center">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              {p.grade} · DNI {p.dni} · Lv {p.level} · {p.points} pts
            </p>
            {ban && (
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      No podrá publicar/comentar hasta que expire o lo
                      desbanees.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {(["1d", "2d", "1w", "1m", "permanent"] as const).map(
                        (k) => (
                          <button
                            key={k}
                            type="button"
            ...
            {user?.id !== p.id && (
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>
                      ¿Eliminar cuenta de {p.full_name}?
                    </AlertDialogTitle>
                    <AlertDialogDescription>
                      Se borrarán todas sus publicaciones, comentarios, libros y
          ...
          <div className="max-h-[500px] overflow-y-auto p-4 text-sm leading-relaxed">
            ...
            {activity.length === 0 ? (
              ...
              activity.slice(0, 100).map((a) => (
                <div
                  key={a.kind + a.id}
                  className="flex gap-3 border-b border-emerald-500/10 py-1.5"
                >
                  ...
                  <span className="shrink-0 text-yellow-300">
                    {a.kind === "post" && (
                      <>
                        <FileText className="inline h-3 w-3" /> POST
                      </>
                    )}
                    {a.kind === "comment" && (
                      <>
                        <MessageCircle className="inline h-3 w-3" /> COMM
                      </>
                    )}
                    {a.kind === "like" && (
                      <>
                        <Heart className="inline h-3 w-3" /> LIKE
                      </>
                    )}
                  </span>
