...
const teacherSchema = z.object({
  codigo_modular: z
    .string()
    .regex(/^\d{10}$/, "El código modular debe tener 10 dígitos"),
  dni: z.string().regex(/^\d{8}$/, "El DNI debe tener 8 dígitos"),
      ...
      if (error) {
        ...
        toast.error(
          m.includes("already")
            ...
            : m.includes("teacher_profiles_codigo_modular_key")
              ? "Ese código modular ya está en uso"
              : m,
        );
        return;
      }
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          {tab === "teacher" && (
            <div>
              <label className="mb-1 block text-sm font-bold">
                Código Modular{" "}
                <span className="text-muted-foreground">(10 dígitos)</span>
              </label>
              <input
                inputMode="numeric"
                maxLength={10}
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Código que identifica al profesor en planilla / PerúEduca.
              </p>
            </div>
          )}
          <div>
            <label className="mb-1 block text-sm font-bold">
              DNI <span className="text-muted-foreground">(8 dígitos)</span>
            </label>
            <input
              inputMode="numeric"
              maxLength={8}
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ej. Ana Pérez Rojas"
              maxLength={120}
              className={inputCls}
            />
          </div>
