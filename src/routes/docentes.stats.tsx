  ...
  useEffect(() => {
    ...
    (async () => {
      const [
        ...
        supabase
          .from("posts")
          .select("category_id, categories!inner(name,color,emoji)")
          .limit(5000),
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (supabase as any)
          .from("page_views")
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Métricas y moderación de la plataforma.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <div className="neo-card p-5">
            ...
            <ul className="mt-3 flex flex-wrap gap-2 text-xs">
              {cats.map((c) => (
                <li
                  key={c.name}
                  className="flex items-center gap-1 rounded-md border-2 border-ink bg-card px-2 py-1 font-bold"
                >
                  <span
          ...
          <div className="neo-card p-5">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button
                onClick={() =>
                  setBarMode((m) => (m === "views" ? "posts" : "views"))
                }
              ...
              >
                {barMode === "views" ? (
                  ...
                  <>
                    <TrendingUp className="h-3 w-3" /> Publicaciones
                  </>
                )}
              </button>
            </div>
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={barMode === "views" ? views : postsPerDay}>
                ...
                <Bar
                  dataKey="count"
                  fill={barMode === "views" ? "#6366f1" : "#22c55e"}
                />
              </BarChart>
            </ResponsiveContainer>
          </div>
                    ...
                    <tr key={u.id} className="hover:bg-muted/40">
                      ...
                      <td className="px-4 py-3 text-right">
                        <Link
                          ...
                          params={{ dni: u.dni }}
                          className="neo-btn rounded-md bg-primary px-2 py-1 text-xs font-bold text-primary-foreground"
                        >
                          Ver detalle
                        </Link>
                      </td>
...
function StatCard({
  ...
  return (
    <div className="neo-card flex items-center gap-3 p-4">
      <div
        className={`flex h-11 w-11 items-center justify-center rounded-lg border-2 border-ink ${color}`}
      >
        {icon}
      </div>
      <div>
