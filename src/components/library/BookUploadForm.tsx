...
export function BookUploadForm({
  ...
  const [stage, setStage] = useState<
    "idle" | "uploading-cover" | "moderating" | "uploading" | "saving"
  >("idle");
  ...
  const form = (
    <form onSubmit={submit} className="space-y-5">
      ...
      <div>
        ...
        {file && (
          <p className="mt-1 text-xs text-muted-foreground">
            {(file.size / 1024 / 1024).toFixed(2)} MB — no ejecutables (.exe,
            .bat, etc.)
          </p>
        )}
      </div>
      ...
      <p className="text-xs text-muted-foreground">
        <ShieldCheck className="mr-1 inline h-3 w-3" /> Portada + metadatos
        serán revisados por IA antes de publicar.
      </p>
    </form>
  );
