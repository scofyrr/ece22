      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          ...
          {isAdmin && (
            <Link
              to="/admin"
              className="rounded-md px-3 py-1.5 text-sm font-semibold hover:bg-muted"
            >
              Admin
            </Link>
          )}
        ...
        <div className="flex items-center gap-2">
          {user && profile ? (
            <>
              <Link
                to="/new"
                className="neo-btn hidden items-center gap-1.5 rounded-lg bg-lemon px-3 py-1.5 text-sm font-bold text-ink sm:inline-flex"
              >
              ...
              <Link
                ...
                className="flex items-center gap-2 rounded-lg border-2 border-ink bg-card px-2 py-1 text-sm font-bold shadow-[3px_3px_0_0_var(--ink)] hover:bg-muted"
              >
                <span className="flex h-7 w-7 items-center justify-center rounded-full bg-coral text-xs font-extrabold text-white">
                  {profile.full_name.charAt(0).toUpperCase()}
                </span>
                <span className="chip ml-1 bg-lemon">
                  <Trophy className="h-3 w-3" /> {profile.points}
                </span>
                {isTeacher && (
                  <GraduationCap
            ...
            <>
              <Link
                to="/login"
                className="rounded-md px-3 py-1.5 text-sm font-bold hover:bg-muted"
              >
                Entrar
              </Link>
              <Link
                to="/register"
                className="neo-btn rounded-lg bg-primary px-3 py-1.5 text-sm font-bold text-primary-foreground"
              >
                Crear cuenta
              </Link>
            </>
